...
const tenantConfig = require('../../../../src/core/providers/config/sqliteTenantStore');
const { canConfigure } = require('../permissions');
const { MODELS } = require('../../../../src/core/providers/transcription/whisperModels');
const { encrypt, isEncrypted } = require('../../../../src/core/utils/crypto');
...
async function execute(interaction) {
  ...
  switch (sub) {
    case 'llm':
      return handleLlm(interaction, tenant);
    case 'stt':
      return handleStt(interaction, tenant);
    case 'channel':
      return handleChannel(interaction, tenant);
    case 'role':
      return handleRole(interaction, tenant);
    case 'show':
      return handleShow(interaction, cfg);
  }
}

async function handleLlm(interaction, tenant) {
  const patch = {};
...
async function handleChannel(interaction, tenant) {
  const channel = interaction.options.getChannel('channel', true);
  tenantConfig.update(tenant, { summary_channel_id: channel.id });
  return interaction.reply({
...
async function handleRole(interaction, tenant) {
  const role = interaction.options.getRole('role');
  tenantConfig.update(tenant, { config_role_id: role ? role.id : null });
  return interaction.reply({
    content: role
