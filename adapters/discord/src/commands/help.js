...
function buildEmbed(topic, cfg) {
  switch (topic) {
    case 'setup':       return setupEmbed();
    case 'join':        return joinEmbed();
    case 'leave':       return leaveEmbed();
    case 'config-show': return configShowEmbed();
    case 'config-llm':  return configLlmEmbed();
    case 'config-stt':  return configSttEmbed();
    case 'config-channel': return configChannelEmbed();
    case 'config-role': return configRoleEmbed();
    case 'overview':
    default:            return overviewEmbed(cfg);
  }
}
...
function overviewEmbed(cfg) {
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Current status', value: buildStatus(cfg) }
    );
}
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: 'Usage', value: '`/config llm [provider:<ollama>] [base_url:<url>] [model:<name>]`' },
      {
        name: 'Options',
        value:
          '• `provider` — currently only `ollama`. More adapters can be added in `src/core/providers/llm/`.\n'
...
function configSttEmbed() {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Supported whisper.cpp models', value: modelsTable },
      {
